 ...
 */
abstract class SAXParser protected() {
  /**
  ...
  def reset(): Unit = {
    ...
    throw new UnsupportedOperationException("This SAXParser, \"" + this.getClass.getName + "\", does not support the reset functionality.")
  ...
  def parse(f: File, hb: HandlerBase): Unit = {
    ...
    val escapedURI = FilePathToURI.filepath2URI(f.getAbsolutePath)
    if (SAXParser.DEBUG) System.out.println("Escaped URI = " + escapedURI)
    val input = new InputSource(escapedURI)
    this.parse(input, hb)
  }
  ...
  def parse(is: InputSource, hb: HandlerBase): Unit = {
    require(is != null, "InputSource cannot be null")
    val parser = this.getParser()
    if (hb != null) {
