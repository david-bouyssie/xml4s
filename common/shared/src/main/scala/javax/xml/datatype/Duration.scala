...
object Duration {
   ...
   */
  private def getCalendarTimeInMillis(cal: Calendar) = cal.getTime.getTime
}

// abstract class Duration extends Equals {
trait Duration extends Equals {
  /**
  ...
  def getXMLSchemaType(): QName = {
    val yearSet = isSet(DatatypeConstants.YEARS)
    val monthSet = isSet(DatatypeConstants.MONTHS)
    val daySet = isSet(DatatypeConstants.DAYS)
    val hourSet = isSet(DatatypeConstants.HOURS)
    val minuteSet = isSet(DatatypeConstants.MINUTES)
    val secondSet = isSet(DatatypeConstants.SECONDS)
    // DURATION
    if (yearSet && monthSet && daySet && hourSet && minuteSet && secondSet) return DatatypeConstants.DURATION
    // DURATION_DAYTIME
    if (!yearSet && !monthSet && daySet && hourSet && minuteSet && secondSet) return DatatypeConstants.DURATION_DAYTIME
    // DURATION_YEARMONTH
    if (yearSet && monthSet && !daySet && !hourSet && !minuteSet && !secondSet) return DatatypeConstants.DURATION_YEARMONTH
    // nothing matches
   ...
   */
  def getYears(): Int = getFieldValueAsInt(DatatypeConstants.YEARS)
   ...
   */
  def getMonths(): Int = getFieldValueAsInt(DatatypeConstants.MONTHS)
   ...
   */
  def getDays(): Int = getFieldValueAsInt(DatatypeConstants.DAYS)
   ...
   *
   */
  def getHours(): Int = getFieldValueAsInt(DatatypeConstants.HOURS)
   ...
   *
   */
  def getMinutes(): Int = getFieldValueAsInt(DatatypeConstants.MINUTES)
   ...
   */
  def getSeconds(): Int = getFieldValueAsInt(DatatypeConstants.SECONDS)
  ...
  def getTimeInMillis(startInstant: Calendar): Long = {
    val cal = startInstant.clone.asInstanceOf[Calendar]
    addTo(cal)
    Duration.getCalendarTimeInMillis(cal) - Duration.getCalendarTimeInMillis(startInstant)
  }
   ...
   */
  def getTimeInMillis(startInstant: Date) = {
    val cal = new GregorianCalendar
    cal.setTime(startInstant)
    this.addTo(cal)
    Duration.getCalendarTimeInMillis(cal) - startInstant.getTime
  }
   ...
   */
  private def getFieldValueAsInt(field: DatatypeConstants.Field): Int = {
    val n = getField(field)
   ...
   */
  def multiply(factor: Int): Duration = multiply(BigDecimal.valueOf(factor))
  ...
  def isLongerThan(duration: Duration): Boolean = {
    compare(duration) == DatatypeConstants.GREATER
  }
  ...
  def isShorterThan(duration: Duration): Boolean = {
    compare(duration) == DatatypeConstants.LESSER
  }
  ...
  override def equals(duration: Any): Boolean = {
    duration match {
      case duration1: Duration => {
        if (duration1 eq this) true
        else compare(duration1) == DatatypeConstants.EQUAL
      }
      case _ => false
    }
  }
  override def canEqual(duration: Any): Boolean = duration.isInstanceOf[Duration]
  ...
  override def toString = {
    ...
    if (getSign() < 0) buf.append('-')
    buf.append('P')
    val years = getField(DatatypeConstants.YEARS).asInstanceOf[BigInteger]
    if (years != null) buf.append(years).append('Y')
    val months = getField(DatatypeConstants.MONTHS).asInstanceOf[BigInteger]
    if (months != null) buf.append(months).append('M')
    val days = getField(DatatypeConstants.DAYS).asInstanceOf[BigInteger]
    if (days != null) buf.append(days).append('D')
    val hours = getField(DatatypeConstants.HOURS).asInstanceOf[BigInteger]
    val minutes = getField(DatatypeConstants.MINUTES).asInstanceOf[BigInteger]
    val seconds = getField(DatatypeConstants.SECONDS).asInstanceOf[BigDecimal]
    if (hours != null || minutes != null || seconds != null) {
      buf.append('T')
      if (hours != null) buf.append(hours).append('H')
      if (minutes != null) buf.append(minutes).append('M')
      if (seconds != null) buf.append(toString(seconds)).append('S')
    }
    buf.toString
  }
   ...
   */
  private def toString(bd: BigDecimal): String = {
    val intString = bd.unscaledValue.toString
    val scale = bd.scale
    if (scale == 0) return intString
