 ...
 * @since 1.5
 */
// abstract class XMLGregorianCalendar extends Equals with Cloneable {
trait XMLGregorianCalendar extends Equals with Cloneable {
  ...
  def getMillisecond(): Int = {
    val fractionalSeconds = getFractionalSecond()
    // is field undefined?
    if (fractionalSeconds == null)
      return DatatypeConstants.FIELD_UNDEFINED

    getFractionalSecond().movePointRight(3).intValue
  }
  ...
  override def equals(obj: Any): Boolean = {
    obj match {
      case calendar: XMLGregorianCalendar => {
        if (calendar eq this) true
        else compare(calendar) == DatatypeConstants.EQUAL
      }
      case _ => false
    }
  }

  override def canEqual(obj: Any): Boolean = obj.isInstanceOf[XMLGregorianCalendar]
  ...
  override def hashCode(): Int = {
    ...
    if (timezone == DatatypeConstants.FIELD_UNDEFINED) timezone = 0

    var gc = this
    if (timezone != 0)
      gc = this.normalize()

    gc.getYear() + gc.getMonth() + gc.getDay() + gc.getHour() + gc.getMinute() + gc.getSecond()
  }
   ...
   * @see #toXMLFormat()
   */
  override def toString: String = toXMLFormat()
