...
object DatatypeFactory {
   ...
   */
    val DATATYPEFACTORY_PROPERTY = "javax.xml.datatype.DatatypeFactory"
  /**
  ...
  val DATATYPEFACTORY_IMPLEMENTATION_CLASS = new String("org.apache.xerces.jaxp.datatype.DatatypeFactoryImpl")
  ...
  def newInstance(): DatatypeFactory = {
    try {
      val factory = FactoryFinder.find(
      ...
      factory match {
        case df: DatatypeFactory => df
        case _ => throw new DatatypeConfigurationException()
      }
    }
    catch {
      case e: FactoryFinder.ConfigurationError =>
  ...
  def newInstance(factoryClassName: String, classLoader: ClassLoader): DatatypeFactory = {
    if (factoryClassName == null) throw new DatatypeConfigurationException("factoryClassName == null")
    val clOrCcl = if (classLoader != null) classLoader else Thread.currentThread.getContextClassLoader

    try {
      val factoryClass = if (clOrCcl != null) clOrCcl.loadClass(factoryClassName)
      else Class.forName(factoryClassName)

      factoryClass.newInstance().asInstanceOf[DatatypeFactory]
    } catch {
 ...
 */
abstract class DatatypeFactory protected() {
  ...
  def newDuration(
    ...
    val realYears = if (years != DatatypeConstants.FIELD_UNDEFINED) BigInteger.valueOf(years.toLong) else null

    // months may not be set
    val realMonths = if (months != DatatypeConstants.FIELD_UNDEFINED) BigInteger.valueOf(months.toLong) else null

    // days may not be set
    val realDays = if (days != DatatypeConstants.FIELD_UNDEFINED) BigInteger.valueOf(days.toLong) else null

    // hours may not be set
    val realHours = if (hours != DatatypeConstants.FIELD_UNDEFINED) BigInteger.valueOf(hours.toLong) else null

    // minutes may not be set
    val realMinutes = if (minutes != DatatypeConstants.FIELD_UNDEFINED) BigInteger.valueOf(minutes.toLong) else null

    // seconds may not be set
    val realSeconds = if (seconds != DatatypeConstants.FIELD_UNDEFINED) BigDecimal.valueOf(seconds.toLong) else null

    newDuration(isPositive, realYears, realMonths, realDays, realHours, realMinutes, realSeconds)
  }
  ...
  def newDurationDayTime(lexicalRepresentation: String): Duration = {
    if (lexicalRepresentation == null) throw new NullPointerException("lexicalRepresentation == null")

    // The lexical representation must match the pattern [^YM]*(T.*)?
    val pos = lexicalRepresentation.indexOf('T')
    val length = if (pos >= 0) pos else lexicalRepresentation.length

    for (i <- 0 until length) {
      val c = lexicalRepresentation.charAt(i)
      require (c != 'Y' && c != 'M', "Invalid dayTimeDuration value: " + lexicalRepresentation)
    }
  ...
  def newDurationDayTime(durationInMilliseconds: Long): Duration = {
    var _durationInMilliseconds = durationInMilliseconds
    ...
    if (_durationInMilliseconds < 0) {
      isPositive = false
      if (_durationInMilliseconds == java.lang.Long.MIN_VALUE) {
    ...
    var value = _durationInMilliseconds
    var milliseconds = (value % 60000L).toInt // 60000 milliseconds per minute

    if (tooLong) milliseconds += 1
    if (milliseconds % 1000 == 0) {
      val seconds = milliseconds / 1000
      value = value / 60000L

      val minutes = (value % 60L).toInt // 60 minutes per hour
      value = value / 60L

      val hours = (value % 24L).toInt // 24 hours per day
      val days = value / 24L

      if (days <= Integer.MAX_VALUE.toLong)
        return newDuration(isPositive, DatatypeConstants.FIELD_UNDEFINED, DatatypeConstants.FIELD_UNDEFINED, days.toInt, hours, minutes, seconds)
      else
    ...
    val seconds = BigDecimal.valueOf(milliseconds, 3)
    value = value / 60000L

    val minutes = BigInteger.valueOf(value % 60L)
    value = value / 60L

    val hours = BigInteger.valueOf(value % 24L)
    value = value / 24L

    val days = BigInteger.valueOf(value)

    newDuration(isPositive, null, null, days, hours, minutes, seconds)
  }
  ...
  def newDurationDayTime(
    ...
    newDuration(
      ...
      null,
      day, hour, minute,
      if (second != null) new BigDecimal(second) else null
    )
  }
  ...
  def newDurationDayTime(isPositive: Boolean, day: Int, hour: Int, minute: Int, second: Int): Duration = {
    newDuration(isPositive, DatatypeConstants.FIELD_UNDEFINED, DatatypeConstants.FIELD_UNDEFINED, day, hour, minute, second)
  }
  ...
  def newDurationYearMonth(lexicalRepresentation: String): Duration = {
    ...
    val length = lexicalRepresentation.length
    for (i <- 0 until length) {
      val c = lexicalRepresentation.charAt(i)
      require(c != 'D' && c != 'T', "Invalid yearMonthDuration value: " + lexicalRepresentation)
    }
  ...
  def newDurationYearMonth(isPositive: Boolean, year: Int, month: Int): Duration = {
    newDuration(isPositive, year, month, DatatypeConstants.FIELD_UNDEFINED, DatatypeConstants.FIELD_UNDEFINED, DatatypeConstants.FIELD_UNDEFINED, DatatypeConstants.FIELD_UNDEFINED)
  }
  ...
  def newXMLGregorianCalendar(
    ...
    val realYear = if (year != DatatypeConstants.FIELD_UNDEFINED) BigInteger.valueOf(year.toLong) else null

    // millisecond may be undefined
    // millisecond must be >= 0 millisecond <= 1000
    var realMillisecond: BigDecimal = null // undefined value
    if (millisecond != DatatypeConstants.FIELD_UNDEFINED) {
      ...
      realMillisecond = BigDecimal.valueOf(millisecond.toLong, 3)
    }

    newXMLGregorianCalendar(realYear, month, day, hour, minute, second, realMillisecond, timezone)
  }
  ...
  def newXMLGregorianCalendarTime(
    ...
    var realMilliseconds: BigDecimal = null
    ...
    if (milliseconds != DatatypeConstants.FIELD_UNDEFINED) {
      ...
      realMilliseconds = BigDecimal.valueOf(milliseconds.toLong, 3)
    }

    newXMLGregorianCalendarTime(hours, minutes, seconds, realMilliseconds, timezone)
  }
}